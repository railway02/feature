#!/usr/bin/env python3
"""Extract frozen api_fullseq_v2 frame, kinetics, pair, flow, and cache data."""
...
CONFIG_DEFAULT = PROJECT / "configs/api_fullseq_v2_pairdata_config.json"
...
TRAIN_OUTPUT_DEFAULT = PROJECT / "outputs/api_fullseq_v2_pairdata/pilot/train"
VALID_OUTPUT_DEFAULT = PROJECT / "outputs/api_fullseq_v2_pairdata/pilot/valid_integrity"
...
class RunLogger:
    def __init__(self, mode: str) -> None:
        ...
        self.path = LOG_ROOT / f"api_fullseq_v2_feature_pilot_{mode}_{stamp}_{os.getpid()}.log"
...
def selected_ids_for_mode(config: dict[str, Any], mode: str) -> tuple[list[str], str]:
    ...
    if mode in {"full_train", "full_valid"}:
        if config["execution_limits"]["pilot_only_for_current_task"]:
...
def manifest_phase_plans(
    ...
    allowed_ids, expected_split = selected_ids_for_mode(config, mode)
    ...
    if manifest["patient_id"].duplicated().any():
        raise AssertionError("Pilot manifest patient_id is not unique")
...
def dry_run_summary(
    ...
    if not filtered:
        expected = (
            config["pilot"]["expected_train_pairs"]
            if mode == "pilot_train"
            else config["pilot"]["expected_valid_integrity_pairs"]
        )
...
def write_failure(stage: str, exc: BaseException) -> None:
    failure_path = REPORT_ROOT / "failure.md"
...
def main() -> int:
    ...
    parser.add_argument("--manifest", default=str(PILOT_MANIFEST))
    parser.add_argument(
        "--mode",
        required=True,
    ...
    parser.add_argument("--max-visual-pairs", type=int, default=3)
    ...
    try:
        ...
        if config_path != CONFIG_DEFAULT.resolve():
            raise AssertionError("Current task requires the frozen default pairdata config")
