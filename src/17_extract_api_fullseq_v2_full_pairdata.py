#!/usr/bin/env python3
"""Extract promoted api_fullseq_v2 Full Train pairdata with frozen science."""
...
CONFIG_DEFAULT = PROJECT / "configs/api_fullseq_v2_full_config.json"
...
class RunLogger:
    def __init__(self, mode: str) -> None:
        ...
        self.path = LOG_ROOT / f"api_fullseq_v2_full_train_{mode}_{stamp}_{os.getpid()}.log"
...
def dry_run_summary(
    ...
    if not filtered:
        if mode == "pilot_train":
            ...
            raise RuntimeError("Full Valid execution is outside the current task")
...
def main() -> int:
    ...
    try:
        ...
        if config_path != CONFIG_DEFAULT.resolve():
            raise AssertionError("Current task requires the Full Train config")
        if args.mode == "full_valid":
            raise RuntimeError("Full Valid is explicitly forbidden for the current task")
        if args.mode == "full_train" and args.max_visual_pairs != 0:
            raise AssertionError(
                "Full Train requires --max-visual-pairs 0; fixed QC is selected internally"
            )
        ...
        if complete_run and not filtered:
            if args.mode == "pilot_train":
                ...
                raise RuntimeError("Full Valid is outside the current task")
