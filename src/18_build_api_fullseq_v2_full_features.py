#!/usr/bin/env python3
"""Build promoted api_fullseq_v2 phase/patient features from frozen pairdata."""
...
CONFIG_PATH = PROJECT / "configs/api_fullseq_v2_full_config.json"
PILOT_MANIFEST = PROJECT / "manifests/api_fullseq_v2_pilot_manifest.csv"
FULL_TRAIN_MANIFEST = PROJECT / "manifests/api_fullseq_v2_full_frozen.csv"
...
def mode_spec(config: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    if mode == "full_valid":
        raise RuntimeError("Full Valid is explicitly forbidden for the current task")
...
def build_report(
    ...
    lines = [
        ...
        "- Labels read: no",
        "- Model training: no",
        f"- Full Train extraction: {'completed stage one' if mode == 'full_train' else 'not started'}",
        "- Full Valid extraction: not started",
        "",
        "## Phase audit",
        "",
        *markdown_table(
...
def write_failure(stage: str, exc: BaseException) -> None:
    ...
    path.write_text(
        "\n".join([
            "# api_fullseq_v2 Full Train feature-build failure",
            "",
            f"- Stage: {stage}",
...
def main() -> int:
    ...
    try:
        ...
        if root_summary["full_valid_started"]:
            raise AssertionError("Full Valid extraction was started")
        ...
        if schema_path != SCHEMA_DEFAULT.resolve():
            raise AssertionError("Promotion/Full Train must use the frozen feature schema")
        if freeze_path != FREEZE_DEFAULT.resolve():
            raise AssertionError("Promotion/Full Train must use the frozen hash manifest")
        ...
        check(
            ...
            (
                ...
                else bool(root_summary["full_train_started"])
            ) and not root_summary["full_valid_started"],
            "promotion reads Pilot; Full mode reads Full Train; Full Valid never starts",
        )
        check(
