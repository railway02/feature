#!/usr/bin/env python3
"""Build api_fullseq_v2 Full Train/Valid features from frozen pairdata."""
...
CONFIG_PATH = PROJECT / "configs/api_fullseq_v2_full_train_valid_config.json"
...
PROMOTION_FEATURE_ROOT = PROJECT / "outputs/api_fullseq_v2_features/promotion_equivalence"
...
def mode_spec(config: dict[str, Any], mode: str) -> dict[str, Any]:
    if mode == "promotion_equivalence":
        return {
            "ids": list(config["pilot"]["train_patient_ids"]),
            "split": "Train",
            "expected_pairs": int(config["pilot"]["expected_train_pairs"]),
            "expected_phases": int(config["promotion_equivalence"]["expected_cpu_phases"]),
            "manifest": PILOT_MANIFEST,
            "phase_default": PROMOTION_FEATURE_ROOT / "train_phase_features.csv",
            "patient_default": PROMOTION_FEATURE_ROOT / "train_patient_features.csv",
            "missing_default": PROMOTION_FEATURE_ROOT / "train_missingness_audit.csv",
            "group_default": PROMOTION_FEATURE_ROOT / "train_feature_group_counts.csv",
            "report_markdown": PROMOTION_FEATURE_ROOT / "train_aggregation_audit.md",
            "report_phase": PROMOTION_FEATURE_ROOT / "train_phase_audit.csv",
        }
    if mode == "full_train":
        ...
        manifest = pd.read_csv(FULL_TRAIN_MANIFEST, dtype={"patient_id": str})
        return {
            "ids": manifest["patient_id"].tolist(),
            "split": "Train",
            "expected_pairs": int(config["full"]["expected_train_pairs"]),
            "expected_phases": int(config["full"]["expected_train_phases"]),
            "manifest": FULL_TRAIN_MANIFEST,
            ...
            "report_markdown": REPORT_ROOT / "train_audit.md",
            "report_phase": REPORT_ROOT / "train_phase_audit.csv",
        }
    if mode == "full_valid":
        if config["execution_limits"]["full_valid_forbidden_for_current_task"]:
            raise RuntimeError("full_valid is forbidden by the production config")
        manifest = pd.read_csv(FULL_VALID_MANIFEST, dtype={"patient_id": str})
        return {
            "ids": manifest["patient_id"].tolist(),
            "split": "Valid",
            "expected_pairs": int(config["full"]["expected_valid_pairs"]),
            "expected_phases": int(config["full"]["expected_valid_phases"]),
            "manifest": FULL_VALID_MANIFEST,
            "phase_default": FEATURE_ROOT / "valid_phase_features.csv",
            "patient_default": FEATURE_ROOT / "valid_patient_features.csv",
            "missing_default": FEATURE_ROOT / "valid_missingness_audit.csv",
            "group_default": FEATURE_ROOT / "valid_feature_group_counts.csv",
            "report_markdown": REPORT_ROOT / "valid_audit.md",
            "report_phase": REPORT_ROOT / "valid_phase_audit.csv",
        }
...
def expected_phase_records(config: dict[str, Any], mode: str) -> list[dict[str, Any]]:
    ...
    if set(manifest["patient_id"]) != set(spec["ids"]):
        raise AssertionError("Expected patient missing from selected frozen manifest")
...
def build_report(
    ...
    lines = [
        ...
        "- Labels read: no",
        "- Model training: no",
        f"- Full Train extraction: {'completed stage one' if mode in {'full_train', 'full_valid'} else 'not started'}",
        f"- Full Valid extraction: {'completed stage one' if mode == 'full_valid' else 'not started'}",
        "",
        "## Phase audit",
        "",
        *markdown_table(
...
def write_failure(stage: str, exc: BaseException) -> None:
    ...
    path.write_text(
        "\n".join([
            "# api_fullseq_v2 Full Train + Full Valid feature-build failure",
            "",
            f"- Stage: {stage}",
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode",
        required=True,
        choices=["promotion_equivalence", "full_train", "full_valid"],
    )
    ...
    try:
        ...
        if args.mode == "full_valid" and not TRAIN_SUCCESS_MARKER.is_file():
            raise AssertionError("Full Valid feature build requires .FULL_TRAIN_SUCCESS")
        ...
        if args.mode == "promotion_equivalence" and root_summary["full_train_started"]:
            raise AssertionError("Promotion aggregation must read frozen Pilot pairdata")
        if args.mode == "full_train" and not root_summary["full_train_started"]:
            raise AssertionError("Full Train feature build requires Full Train pairdata")
        if args.mode == "full_train" and root_summary["full_valid_started"]:
            raise AssertionError("Full Train pairdata root is contaminated by Full Valid")
        if args.mode == "full_valid" and (
            root_summary["full_train_started"] or not root_summary["full_valid_started"]
        ):
            raise AssertionError("Full Valid feature build requires isolated Full Valid pairdata")
        ...
        success_marker = (
            PROMOTION_FEATURE_ROOT / ".AGGREGATION_SUCCESS"
            if args.mode == "promotion_equivalence" else None
        )
        freeze_path = Path(args.freeze_output).resolve()

        if args.resume and success_marker is not None and success_marker.is_file():
            print(f"Already complete: {success_marker}")
            return 0
        targets = [
            phase_output, patient_output, missing_output, group_output,
            report_markdown, report_phase,
        ]
        if success_marker is not None:
            targets.append(success_marker)
        if not schema_path.is_file() or not freeze_path.is_file():
            raise FileNotFoundError("Frozen Pilot schema and hash manifest are required")
        if schema_path != SCHEMA_DEFAULT.resolve():
            raise AssertionError("Full Train/Valid must use the frozen feature schema")
        if freeze_path != FREEZE_DEFAULT.resolve():
            raise AssertionError("Full Train/Valid must use the frozen hash manifest")
        conflicts = [] if args.resume else [path for path in targets if path.exists()]
        if conflicts:
        ...
        if len(phase_frame) != spec["expected_phases"]:
            raise AssertionError(
                f"Phase feature count expected {spec['expected_phases']}, got {len(phase_frame)}"
            )
        ...
        if schema["emit_legacy_ablation"] != args.emit_legacy_ablation:
            raise AssertionError("Legacy-ablation setting differs from frozen Train schema")
        if schema["phase_features"] != generated_schema["phase_features"]:
            raise AssertionError("Generated phase schema differs from frozen Train schema")
        if schema["patient_features"] != generated_schema["patient_features"]:
            raise AssertionError("Generated patient schema differs from frozen Train schema")
        ...
        if args.mode in {"promotion_equivalence", "full_train"}:
            row571 = phase_audit[
        ...
        if args.mode == "full_train":
            check(
                "full_train_post_only_count",
                len(post_only) == int(config["full"]["expected_train_post_only_patients"]),
                f"expected={config['full']['expected_train_post_only_patients']} actual={len(post_only)}",
            )
        if args.mode == "full_valid":
            check(
                "full_valid_post_only_count",
                len(post_only) == int(config["full"]["expected_valid_post_only_patients"]),
                f"expected={config['full']['expected_valid_post_only_patients']} actual={len(post_only)}",
            )
        check(
            "labels_not_read",
        ...
        check(
            ...
            (
                ...
                if args.mode == "promotion_equivalence"
                else (
                    bool(root_summary["full_train_started"])
                    if args.mode == "full_train"
                    else bool(root_summary["full_valid_started"])
                )
            ),
            "promotion reads Pilot; Full Train and Full Valid read isolated stage-one roots",
        )
        check(
        ...
        REPORT_ROOT.mkdir(parents=True, exist_ok=True)
        for path in [phase_output, patient_output, missing_output, group_output, report_phase]:
        ...
        frozen_assertions = verify_freeze(freeze_path, config)
        assertions.extend(frozen_assertions)
        if not all(item["passed"] for item in frozen_assertions):
            raise AssertionError("Frozen Pilot hash changed during promoted aggregation")
