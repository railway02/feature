...
def main() -> int:
    ...
    train_meta = pd.read_csv(
        reports / "train_oof_predictions.csv", dtype={"patient_id": str}
    )
    valid_meta = pd.read_csv(
        reports / "valid_predictions.csv", dtype={"patient_id": str}
    )
    ...
    for variant in VARIANTS:
        ...
        for split in ("Train_OOF", "Valid"):
            ...
            roi = metric_frame[
                (metric_frame["model"] == f"pred_roi_Logistic_{variant}")
                & (metric_frame["split"] == split)
            ].iloc[0]
            ...
            comparisons[variant][split].update(
                {
                    f"roi_{key}": float(roi[key])
                    for key in ("auroc", "auprc", "brier")
                }
            )
            comparisons[variant][split].update(
                {
                    f"roi_minus_whole_{key}": float(roi[key] - whole[key])
                    for key in ("auroc", "auprc", "brier")
                }
            )
    summary = {
        "version": "matched_logistic_variants_1",
        "task": "adverse_patient",
