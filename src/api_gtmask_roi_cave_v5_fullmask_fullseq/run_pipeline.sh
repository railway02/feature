...
CODE=${CODE:-$ROOT/code/api_gtmask_roi_cave_v5_fullmask_fullseq}
CONFIG=${CONFIG:-$ROOT/configs/api_gtmask_roi_cave_v5_fullmask_fullseq.json}
PY=${PYTHON:-python}
...
case "$stage" in
  ...
  status)
    ...
    ls -l "$ROOT/manifests/api_gtmask_roi_cave_v5_fullmask_fullseq/.STAGE1_ELIGIBLE_SUCCESS.json" 2>/dev/null || true
    echo "== eligible featurebank =="
    ls -l "$ROOT/outputs/api_gtmask_roi_cave_v5_fullmask_fullseq/cave_local_eligible_featurebank/.ELIGIBLE_FEATUREBANK_SUCCESS.json" 2>/dev/null || true
    echo "== running processes =="
    pgrep -af 'api_gtmask_roi_cave_v5_fullmask_fullseq' || true
    ;;
  *)
