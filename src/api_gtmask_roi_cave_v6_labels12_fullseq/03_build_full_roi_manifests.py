...
def evaluate_phase(mapped: dict[str, Any], roi_cfg: dict, whole_root: Path) -> tuple[dict[str, Any], dict[str, Any] | None, dict[str, Any] | None, dict[str, Any] | None]:
    ...
    if selected_labels != [1, 2]:
        raise ValueError(
            f"This pipeline requires foreground_labels=[1, 2], got {selected_labels}"
        )
    foreground = np.isin(labels, selected_labels)
    labels_present = sorted(
        int(value) for value in np.unique(labels).tolist() if int(value) != 0
    )
    selected_set = set(selected_labels)
    labels_ignored = [
        value for value in labels_present if value not in selected_set
    ]
    ignored_foreground = (labels != 0) & ~foreground
    cov.update({
        "foreground_rule": "labels_in_1_2",
        "selected_labels": json.dumps(selected_labels, separators=(",", ":")),
        "labels_present": json.dumps(labels_present, separators=(",", ":")),
        "labels_ignored": json.dumps(labels_ignored, separators=(",", ":")),
        "selected_foreground_pixels": int(foreground.sum()),
        "ignored_foreground_pixels": int(ignored_foreground.sum()),
    })
    if not bool(foreground.any()):
        cov["local_exclusion_reason"] = "empty_selected_foreground_labels_1_2"
        return cov, None, None, None
...
def main() -> int:
    ...
    atomic_csv(coverage, manifests / "labels12_phase_coverage_all.csv")
    ...
    lock = {
        ...
        "roi_rule": {
            "foreground": "numpy.isin(segmentation, [1, 2])",
            "foreground_rule": "labels_in_1_2",
            "selected_labels": [1, 2],
