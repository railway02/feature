...
def main() -> int:
    ...
    if target_rule != "labels_1_2_for_roi_only" or selected_labels != [1, 2]:
        raise ValueError(
            f"Invalid labels12 ROI config: rule={target_rule}, labels={selected_labels}"
        )
    roi_branch = str(roi_cfg.get("branch", "manual_gt_labels12_fullseq"))
    ...
    def roi_load(paths, num_workers=4):
        ...
        field = "fallback_bbox" if active_mode.get(key) == "fallback" else "expanded_bbox"
    ...
    def cache_state(args_inner, plan, row):
        ...
        valid_bboxes = {str(row["expanded_bbox"]), str(row["fallback_bbox"])}
    ...
    def roi_process(args_inner, extractor, v3, plan, provenance, schema_path):
        ...
        if state == "valid" and not overwrite:
            return original_process(args_inner, extractor, v3, plan, provenance, schema_path)
        if state == "partial" and not overwrite:
            quarantine_partial(directory, row["phase_uid"], reason)

        mode = "primary"
        try:
            result = run_once(args_inner, extractor, v3, plan, provenance, schema_path, mode)
        except (AssertionError, RuntimeError, ValueError) as exc:
            enabled = bool(cfg.get("roi", {}).get("activity_fallback_enabled", True))
            if not enabled or not is_activity_error(exc) or row["fallback_bbox"] == row["expanded_bbox"]:
                raise
            directory = cave._phase_output_dir(args_inner.output_root, plan)
        ...
        directory = cave._phase_output_dir(args_inner.output_root, plan)
        used_bbox = row["fallback_bbox"] if mode == "fallback" else row["expanded_bbox"]
        metadata_path = directory / "metadata.json"
        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        metadata["roi"] = {
