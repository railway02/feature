...
CODE=${CODE:-$ROOT/code/api_gtmask_roi_cave_v6_labels12_fullseq}
CONFIG=${CONFIG:-$ROOT/configs/api_gtmask_roi_cave_v6_labels12_fullseq.json}
PY=${PYTHON:-python}
...
case "$stage" in
  index)
    ...
    ;;
  discover)
    "$PY" "$CODE/01_discover_masks.py" --config "$CONFIG"
    ;;
  map)
    "$PY" "$CODE/02_map_masks_to_phases.py" --config "$CONFIG"
    ;;
  roi-eligible)
    "$PY" "$CODE/03_build_full_roi_manifests.py" --config "$CONFIG"
    ;;
  qa)
    "$PY" "$CODE/04_make_roi_qa.py" --config "$CONFIG"
    ;;
  validate-stage1-eligible)
    "$PY" "$CODE/05_validate_stage1.py" --config "$CONFIG"
    ;;
  stage1)
    "$PY" "$CODE/00_index_source_phases.py" --config "$CONFIG"
    "$PY" "$CODE/01_discover_masks.py" --config "$CONFIG"
  ...
  status)
    ...
    ls -l "$ROOT/manifests/api_gtmask_roi_cave_v6_labels12_fullseq/.STAGE1_ELIGIBLE_SUCCESS.json" 2>/dev/null || true
    echo "== eligible featurebank =="
    ls -l "$ROOT/outputs/api_gtmask_roi_cave_v6_labels12_fullseq/cave_local_eligible_featurebank/.ELIGIBLE_FEATUREBANK_SUCCESS.json" 2>/dev/null || true
    echo "== running processes =="
    pgrep -af 'api_gtmask_roi_cave_v6_labels12_fullseq' || true
    ;;
  *)
