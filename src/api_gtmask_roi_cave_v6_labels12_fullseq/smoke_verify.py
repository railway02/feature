...
def main() -> int:
    ...
    if frozen.get("foreground_rule") != "labels_in_1_2":
        raise AssertionError("Frozen CAVE config does not declare labels_in_1_2")
    if frozen.get("selected_labels") != [1, 2]:
        raise AssertionError("Frozen CAVE config does not declare selected_labels=[1,2]")
    if "api_gtmask_roi_cave_v5_fullmask_fullseq" in str(smoke_root):
        raise AssertionError("v6 smoke output points to old v5 root")
    ...
    for success_path in success_files:
        ...
        expected = str(record.expanded_bbox) if not row["fallback_used"] else str(record.fallback_bbox)
        if roi_meta.get("target_rule") != "labels_1_2_for_roi_only":
            failures.append(f"{phase_uid}: target_rule 不是 labels_1_2_for_roi_only")
        if roi_meta.get("foreground_rule") != "labels_in_1_2":
            failures.append(f"{phase_uid}: foreground_rule 不是 labels_in_1_2")
