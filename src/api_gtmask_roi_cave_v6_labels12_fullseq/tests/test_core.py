...
class QaOverlayTests(unittest.TestCase):
    def test_overlay_marks_contour_tight_and_roi_boxes(self):
        ...
        image = module.overlay(frame, labels, (8, 18, 27, 32), (10, 20, 25, 30), (0, 8, 40, 42), (-8, 0, 48, 50))
        self.assertEqual(image.shape, (64, 64, 3))
        # 橙色旧 bbox/绿色 ROI 边框/红色新 tight bbox 都必须真实画上
        self.assertTrue(((image[..., 0] == 0) & (image[..., 1] == 165) & (image[..., 2] == 255)).any())
...
class EligibilityTests(unittest.TestCase):
    ...
    def test_eligible_phase_produces_roi(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            cov, roi_row, morph, view = module.evaluate_phase(mapped, {"foreground_labels": [1, 2], "target_rule": "labels_1_2_for_roi_only"}, whole)
    ...
    def test_labels_3_to_6_do_not_expand_labels12_bbox(self):
        labels = np.zeros((80, 80), dtype=np.int16)
        labels[20:25, 20:25] = 1
        labels[30:35, 30:35] = 2
        labels[65:75, 65:75] = 6
        bbox = bbox_from_mask(np.isin(labels, [1, 2]))
        self.assertEqual(bbox, (20, 20, 35, 35))

    def test_label6_only_is_excluded_without_nonzero_fallback(self):
        module = import_script("03_build_full_roi_manifests.py")
        with tempfile.TemporaryDirectory() as directory:
            mapped, whole = self._fixture(directory)
            labels = np.zeros((64, 64), dtype=np.int16)
            labels[20:30, 10:25] = 6
            write_nifti(Path(mapped["mask_path"]), labels)
            cov, roi_row, _, _ = module.evaluate_phase(mapped, {"foreground_labels": [1, 2], "target_rule": "labels_1_2_for_roi_only"}, whole)
            self.assertEqual(cov["local_exclusion_reason"], "empty_selected_foreground_labels_1_2")
    ...
    def test_missing_whole_metadata_excludes(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            mapped, whole = self._fixture(directory)
            cov, roi_row, _, _ = module.evaluate_phase(mapped, {"foreground_labels": [1, 2], "target_rule": "labels_1_2_for_roi_only"}, whole / "nonexistent")
