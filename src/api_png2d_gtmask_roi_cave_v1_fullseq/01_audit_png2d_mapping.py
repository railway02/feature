...
def main() -> int:
    ...
    if correlations.isna().any() or (
        correlations < float(mapping_cfg["min_accepted_identity_correlation"])
    ).any():
        failures.append("accepted mapping has invalid/below-threshold correlation")
    ...
    if len(orientation_audit):
        if not orientation_audit["best_transform"].eq("identity").all():
            failures.append("weak accepted mapping has non-identity best transform")
        if (pd.to_numeric(orientation_audit["identity_score"]) < float(
            mapping_cfg["min_accepted_identity_correlation"]
        )).any():
            failures.append("weak accepted identity score below threshold")
