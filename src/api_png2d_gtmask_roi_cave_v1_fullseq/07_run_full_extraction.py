...
def stage_resume_links(
    ...
    """Expose consolidated valid phases to workers without copying/overwriting."""
    linked = 0
    for shard_index, items in enumerate(shards):
        for uid in items:
            row = manifest_by_uid[uid]
            for phase in ("pre", "post"):
                if not as_bool(row.get(f"can_run_{phase}")):
                    continue
                relative = (
                    Path(split.casefold())
                ...
                target = shard_base / f"shard_{shard_index:02d}" / relative
                if not (source / ".SUCCESS.json").is_file() or target.exists():
                    continue
                target.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    resume_links = stage_resume_links(
        output, shard_base, args.split, shards, manifest_by_uid
    )
    ...
    # Preserve every valid phase even when a sibling shard fails. Resume then
    # schedules only incomplete series and never overwrites successful caches.
    # A shard can contain hundreds of valid phase caches and still exit 2
    # because one phase failed. Consolidation itself only moves directories
    # with a valid .SUCCESS.json, so every shard is safe to scan.
    phase_bearing_shard_roots = list(shard_roots)
    consolidation = (
        consolidate(phase_bearing_shard_roots, output, args.split)
        if phase_bearing_shard_roots
        else {"moved": 0, "duplicate": 0}
    )
        ...
        atomic_json(
            {
                ...
                "status": "failed",
                "shards_scanned_for_valid_phase_caches": len(phase_bearing_shard_roots),
    ...
    summary = {
        ...
        "resume_phase_links": resume_links,
