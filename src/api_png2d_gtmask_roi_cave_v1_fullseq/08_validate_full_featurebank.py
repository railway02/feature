...
def load_candidate_payload(directory: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "provenance": {
            ...
            "roi_pipeline_version": str(success.get("roi_pipeline_version", "")),
            "target_rule": str(success.get("target_rule", "")),
            "foreground_rule": str(success.get("foreground_rule", "")),
            "selected_labels": str(success.get("selected_labels", "")),
            ...
            "roi_pipeline_version_meta": str(roi_meta.get("roi_pipeline_version", "")),
            "roi_target_rule": str(roi_meta.get("target_rule", "")),
            "roi_foreground_rule": str(roi_meta.get("foreground_rule", "")),
            "roi_selected_labels": str(roi_meta.get("selected_labels", "")),
...
def consolidate_schemas(
    ...
    for split in splits:
        ...
        split_details[split] = {
            ...
            "byte_sha256_values": sorted(byte_groups),
            "schema_sha256": str(chosen_payload.get("schema_sha256", "")),
            "allowed_schema_sha256_values": sorted({
                str(payload.get("schema_sha256", ""))
                for _, payload in payloads
                if str(payload.get("schema_sha256", ""))
            }),
            "frozen_config_hash": str(
                chosen_payload.get("frozen_config_hash", "")
            ),
            "frozen_config_hash_values": sorted({
                str(payload.get("frozen_config_hash", ""))
                for _, payload in payloads
            }),
...
def consolidate_from_shards(
    ...
    for split in splits:
        ...
        schema_candidates_by_split[split].extend(
            (reports / "resume_schema_archive").glob(
                f"{split.casefold()}_*.json"
            )
        )
    ...
    return {
        ...
        "allowed_split_schema_self_sha256": {
            split: details.get("allowed_schema_sha256_values", [])
            for split, details in schema_audit.get("splits", {}).items()
        },
        ...
        for row in expected.to_dict("records"):
            ...
            if uid in exclusions:
                ...
                try:
                    ...
                    if str(success.get("phase_uid", "")) != uid:
                    ...
                    elif str(success.get("roi_pipeline_version", "")) != str(cfg["version"]):
                        status, reason = "invalid", "roi_pipeline_version_mismatch"
                        failures.append(f"ROI pipeline version 错误：{uid}")
                    elif success.get("annotation_source") != "png_2d_gt":
                        status, reason = "invalid", "annotation_source_mismatch"
                        failures.append(f"annotation source 错误：{uid}")
                    elif str(success.get("effective_mask_array_sha256", "")) != str(
                        row["effective_mask_array_sha256"]
                    ):
                        status, reason = "invalid", "effective_mask_sha256_mismatch"
                        failures.append(f"映射后 Mask hash 错误：{uid}")
                    elif success.get("target_rule") != "png2d_gt_labels_1_2_nonzero":
                        status, reason = "invalid", "target_rule_mismatch"
                        failures.append(f"ROI target rule 错误：{uid}")
                    elif success.get("foreground_rule") != "labels_in_1_2_equivalent_nonzero":
                        status, reason = "invalid", "foreground_rule_mismatch"
                        failures.append(f"ROI foreground rule 错误：{uid}")
                    elif success.get("selected_labels") != [1, 2]:
                        status, reason = "invalid", "selected_labels_mismatch"
                        failures.append(f"ROI selected labels 错误：{uid}")
                    elif str(success.get("feature_schema_sha256", "")) not in set(
                        consolidation.get(
                            "allowed_split_schema_self_sha256", {}
                        ).get(
                            split,
                        ...
                        if str(roi_meta.get("phase_uid", "")) != uid:
                            status, reason = "invalid", "metadata_roi_phase_uid_mismatch"
                            failures.append(f"metadata ROI phase_uid 错误：{uid}")
                        elif str(roi_meta.get("mask_sha256", "")) != str(row["mask_sha256"]):
                            status, reason = "invalid", "metadata_roi_mask_sha256_mismatch"
                            failures.append(f"metadata ROI Mask hash 错误：{uid}")
                        elif str(roi_meta.get("roi_pipeline_version", "")) != str(cfg["version"]):
                            status, reason = "invalid", "metadata_roi_pipeline_version_mismatch"
                            failures.append(f"metadata ROI pipeline version 错误：{uid}")
                        elif roi_meta.get("annotation_source") != "png_2d_gt":
                            status, reason = "invalid", "metadata_roi_annotation_source_mismatch"
                            failures.append(f"metadata ROI annotation source 错误：{uid}")
                        elif str(roi_meta.get("effective_mask_array_sha256", "")) != str(
                            row["effective_mask_array_sha256"]
                        ):
                            status, reason = "invalid", "metadata_roi_effective_mask_sha256_mismatch"
                            failures.append(f"metadata ROI 映射后 Mask hash 错误：{uid}")
                        elif roi_meta.get("target_rule") != "png2d_gt_labels_1_2_nonzero":
                            status, reason = "invalid", "metadata_roi_target_rule_mismatch"
                            failures.append(f"metadata ROI target rule 错误：{uid}")
                        elif roi_meta.get("foreground_rule") != "labels_in_1_2_equivalent_nonzero":
                            status, reason = "invalid", "metadata_roi_foreground_rule_mismatch"
                            failures.append(f"metadata ROI foreground rule 错误：{uid}")
                        elif roi_meta.get("selected_labels") != [1, 2]:
                            status, reason = "invalid", "metadata_roi_selected_labels_mismatch"
                            failures.append(f"metadata ROI selected labels 错误：{uid}")
                except Exception as exc:
