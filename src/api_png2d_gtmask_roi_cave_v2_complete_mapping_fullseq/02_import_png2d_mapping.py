...
def main() -> int:
    ...
    for source_row in source.to_dict("records"):
        ...
        if mapped is not None:
            base.update({
                ...
                "mapping_reason": (
                    mapped.get("manual_confirmation_note", "")
                    if mapped["mapping_method"] == "manual_visual_identity_confirmed"
                    else ""
                ),
                "manual_confirmation_note": mapped.get("manual_confirmation_note", ""),
    ...
    if (
        len(accepted_full) != int(mapping_cfg["expected_accepted"])
        or len(review_full) != int(mapping_cfg["expected_unresolved"])
        or len(missing_full) != len(full) - int(mapping_cfg["expected_accepted"])
    ):
        failures.append(
    ...
    summary = {
        ...
        "mapping_method_counts": accepted_full["mapping_method"].value_counts().to_dict(),
