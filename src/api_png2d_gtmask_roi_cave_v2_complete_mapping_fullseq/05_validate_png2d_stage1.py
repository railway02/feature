...
def main() -> int:
    ...
    required_roi_columns = {
        ...
        "extended_fallback_bbox", "mapping_method", "manual_confirmation_note",
    }
    ...
    allowed_orientation_status = {
        "identity_mean_verified",
        "manual_visual_identity_confirmed",
    }
    if not set(roi["orientation_status"]).issubset(allowed_orientation_status):
        failures.append("eligible ROI has unsupported orientation status")
    manual_roi = roi["mapping_method"].eq("manual_visual_identity_confirmed")
    if not roi.loc[manual_roi, "orientation_status"].eq(
        "manual_visual_identity_confirmed"
    ).all():
        failures.append("manual mapping lost manual orientation provenance")
    if roi.loc[manual_roi, "manual_confirmation_note"].str.strip().eq("").any():
        failures.append("manual mapping lost confirmation note")
    if not roi.loc[~manual_roi, "orientation_status"].eq(
        "identity_mean_verified"
    ).all():
        failures.append("automated mapping is not identity-mean verified")
    ...
    for bad_uid in (
        "Train__631324__main__b28b7af693::pre",
        "Train__661784__R-C6__4c4bfe5617::pre",
    ):
        bad = coverage[coverage["phase_uid"] == bad_uid]
        if len(bad) != 1 or not str(
            bad.iloc[0]["local_exclusion_reason"]
        ).startswith("nonuniform_scale_not_allowed"):
            failures.append(
                f"{bad_uid} is not explicitly excluded for nonuniform scale"
            )
