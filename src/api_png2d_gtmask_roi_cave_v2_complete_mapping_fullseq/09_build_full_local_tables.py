...
def main() -> int:
    ...
    # The frozen table builder preserves every series row in the supplied
    # manifest, including a fully unavailable series as two all-NaN phase
    # vectors. Keep those rows for explicit availability auditing; the strict
    # Pre+Post task builder later requires both finite phase embeddings.
    runnable_mask = (
        feature_available_manifest["can_run_pre"].map(as_bool)
        | feature_available_manifest["can_run_post"].map(as_bool)
    )
    expected_table_manifest = feature_available_manifest.copy()
