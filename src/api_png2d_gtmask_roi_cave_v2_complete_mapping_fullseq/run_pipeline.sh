...
CODE=${CODE:-$ROOT/code/api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq}
CONFIG=${CONFIG:-$ROOT/configs/api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq.json}
PY=${PYTHON:-python}
...
case "$stage" in
  ...
  stage1)
    ...
    ;;
  reuse-v1)
    "$PY" "$CODE/06_seed_v1_featurebank.py" --config "$CONFIG"
    ;;
  extract-supplement)
    "$CAVE_PY" "$CODE/07_run_full_extraction.py" --config "$CONFIG" --split Train --series-uids-file "$ROOT/reports/api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq/supplement_series_train.txt"
    "$CAVE_PY" "$CODE/07_run_full_extraction.py" --config "$CONFIG" --split Valid --series-uids-file "$ROOT/reports/api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq/supplement_series_valid.txt"
    ;;
  smoke-train)
    extra=()
  ...
  status)
    ...
    ls -l "$ROOT/manifests/api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq/.STAGE1_ELIGIBLE_SUCCESS.json" 2>/dev/null || true
    echo "== eligible featurebank =="
    ls -l "$ROOT/outputs/api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq/cave_local_eligible_featurebank/.ELIGIBLE_FEATUREBANK_SUCCESS.json" 2>/dev/null || true
    echo "== running processes =="
    pgrep -af 'api_png2d_gtmask_roi_cave_v2_complete_mapping_fullseq' || true
    ;;
  *)
    echo "Usage: $0 {index|discover|map|roi-eligible|qa|validate-stage1-eligible|stage1|reuse-v1|extract-supplement|smoke-train|smoke-valid|extract-train-eligible|extract-valid-eligible|extract-all-eligible|validate-features-eligible|table-train-eligible|table-valid-eligible|build-tables-eligible|all|status}" >&2
    exit 2
    ;;
esac
